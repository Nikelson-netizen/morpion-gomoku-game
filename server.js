...
socket.on("createTournament", ({ name, code, playerName }) => {
  ...
  io.emit("tournamentUpdated", tournaments.get(code));
...
socket.on("joinTournament", ({ code, playerName }) => {
  ...
  // Maximum 10 players
if (tournament.players.length >= 10 && !tournament.players.includes(playerName)) {
    socket.emit("tournamentError", {
